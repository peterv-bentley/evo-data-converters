...
def convert_omf_blockmodel_attributes_to_columns(
    blockmodel: omf_python.Element, reader: omf_python.Reader, table: pa.Table, location: omf_python.Location
) -> pa.Table:
    for attribute in blockmodel.attributes():
        if attribute.location != location:
            if location == omf_python.Location.Subblocks and attribute.location == omf_python.Location.Primitives:
...
def convert_blockmodel_attribute(
    reader: omf_python.Reader, attribute: omf_python.Attribute, used_column_names: list[str] = []
) -> Optional[pa.Table]:
    attribute_data = attribute.get_data()

    attribute_name = _unique_attribute_name(attribute.name, used_column_names)

    if isinstance(attribute_data, omf_python.AttributeDataCategory):
    ...
    elif isinstance(attribute_data, omf_python.AttributeDataBoolean):
    ...
    elif isinstance(attribute_data, omf_python.AttributeDataNumber):
